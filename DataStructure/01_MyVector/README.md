...
- **원소 타입의 이동과 복사를 고려한 재할당**<br>(복사시는 trivially_copyable 가능시 memcpy를 통한 블록 단위 복사 사용 / 이동시는 move_if_noexcept 사용을 통한 강한 예외 보장)
...
```powershell
# int 벤치마크
MyVector.exe --benchmark 1000000 10 int

# std::string 벤치마크
MyVector.exe --benchmark 10000 5 string
```

첫 번째 숫자는 각 방식에 추가할 원소 수이고, 두 번째 숫자는 각 방식을 반복 측정할 횟수입니다. 마지막 인자는 원소 타입으로 `int` 또는 `string`을 선택합니다. 원소 타입을 생략하면 `int`, 반복 횟수를 생략하면 10회를 기본값으로 사용합니다.
...
원소 1,000,000개를 각 방식에서 반복 측정했습니다.

| 방식 | 평균 | 중앙값 | 최소 | 최대 | 재할당 | 최종 용량 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| MyVector Geometric Growth 1.5 (기본값) | 3,120μs | 2,992μs | 2,481μs | 4,058μs | 34회 | 1,049,869 |
| MyVector Geometric Growth 2.0 (변경값) | 2,093μs | 1,875μs | 1,704μs | 3,478μs | 20회 | 1,048,576 |
| MyVector Linear Growth | 41,028,674μs | 41,154,850μs | 39,977,743μs | 42,396,184μs | 100,000회 | 1,000,001 |
| `std::vector` | 3,151μs | 2,858μs | 2,650μs | 4,201μs | 35회 | 1,049,869 |
...
이론적으로 Factor 1.5는 Factor 2.0보다 약 **1.5배** 많은 원소를 누적해서 이동한다. 1,000,000개의 `int`를 삽입한 측정에서도 Factor 1.5는 34회, Factor 2.0은 20회의 재할당이 발생했다.

| Growth Factor | 중앙값 | 재할당 | 최종 Capacity |
| --- | ---: | ---: | ---: |
| 1.5 | 2,992μs | 34회 | 1,049,869 |
| 2.0 | 1,875μs | 20회 | 1,048,576 |

두 방식의 최종 Capacity는 약 105만 개로 거의 같지만 Factor 2.0은 더 적은 횟수로 확장하고, 재할당 과정에서 기존 원소를 누적해서 이동하는 양도 적다. 중앙값의 비율은 다음과 같다.

$$\frac{2992}{1875} \approx 1.60$$

실제 시간 차이는 약 1.60배로, 누적 이동량을 통해 예상한 약 1.5배와 유사했다. 이번 입력 크기인 1,000,000개가 Factor 2.0의 최종 Capacity인 1,048,576에 가까웠기 때문에, Factor 2.0은 재할당 횟수를 줄이면서도 추가 메모리 점유가 커지지 않는 유리한 조건이었다.

Growth Factor가 크면 일반적으로 재할당 횟수와 누적 이동량은 감소하지만, 최종 Capacity는 입력 크기가 각 확장 경계의 어느 위치에 도달했는지에 따라 달라진다. 따라서 큰 Factor가 항상 더 많은 메모리를 점유하거나 일정한 비율만큼 빠르다고 일반화할 수는 없다.
...
| 자료형 | 원소 수 | Linear | Factor 1.5 | Factor 2.0 |
| --- | ---: | ---: | ---: | ---: |
| `int` | 1,000,000 | 41,154,850μs | 2,992μs | 1,875μs |
| `string` | 10,000 | 41,581μs | 688μs | 607μs |

1,000,000개의 `int`를 삽입했을 때 Linear Growth는 Factor 1.5보다 약 13,755배 느렸다.

$$\frac{41,154,850}{2,992} \approx 13,755$$

Linear Growth는 100,000번의 재할당이 발생한 반면 Factor 1.5는 34번만 재할당했다. 입력 크기가 증가하면서 Linear Growth의 $O(N^2)$ 누적 재배치 비용과 Geometric Growth의 $O(N)$ 누적 재배치 비용 차이가 명확하게 나타났다.
...
<details>
<summary><strong>3. STL과의 벤치마킹 결과</strong></summary>

1,000,000개의 `int`를 삽입한 결과, 동일한 Growth 정책을 사용하는 MyVector Factor 1.5와 `std::vector`는 유사한 성능을 보였다.

| 구현 | 평균 | 중앙값 | 재할당 | 최종 Capacity |
| --- | ---: | ---: | ---: | ---: |
| MyVector Factor 1.5 | 3,120μs | 2,992μs | 34회 | 1,049,869 |
| MyVector Factor 2.0 | 2,093μs | 1,875μs | 20회 | 1,048,576 |
| `std::vector` | 3,151μs | 2,858μs | 35회 | 1,049,869 |

Factor 1.5와 `std::vector`의 중앙값 차이는 약 4.7%였으며 평균값도 유사했다.

$$\frac{2992}{2858} \approx 1.047$$

기존 10,000개 측정에서는 MyVector Factor 1.5가 43μs, `std::vector`가 14μs로 약 3배의 차이를 보였다. 그러나 측정 규모를 1,000,000개로 늘리자 두 구현의 성능이 비슷해졌다. 따라서 기존 결과는 수십 마이크로초의 짧은 측정 구간에서 고정 비용과 실행 환경의 변동이 상대적으로 크게 반영된 결과로 판단된다.

재할당 횟수가 한 번 다른 것은 MyVector가 Capacity 1을 생성 시점에 미리 할당하고 해당 최초 할당이 측정 시작 전에 이루어지는 반면, `std::vector`는 Capacity 0에서 시작하여 최초 할당이 측정 구간에 포함되기 때문이다. 두 구현은 최종 Capacity가 같고 이후의 Growth 정책도 실질적으로 동일하다.

Factor 2.0은 재할당 횟수가 20회로 줄어 Factor 1.5와 `std::vector`보다 빠르게 측정되었다. 이는 STL보다 구현 자체가 항상 빠르다는 의미가 아니라, 이번 입력에서 더 큰 Growth Factor가 누적 재배치량을 줄인 결과이다.
