...
100,000개의 고유 정수를 삽입·탐색·삭제하는 테스트에서 stl과 벤치마크시 삽입과 삭제에서는 비슷한 시간을 보이지만 탐색에서는 커스텀 구현이 2배 정도 빠른 결과를 보입니다.

이는 실제 커스텀 구현이 빠르다기 보다는 stl의 경우 노드가 아닌 이터레이터의 결과를 반환하지만 해시테이블의 경우는 노드 자체를 반환하여 그런것으로 추정 됩니다.

또한 탐색의 경우, 두 구현 모두 삽입·삭제의 케이스 보다 빠른 경향을 보이는데 이는 메모리 할당/해제 작업의 영향으로 보입니다.

삽입이 삭제보다 더 큰 시간이 걸리는 것은 컨테이너의 버킷 배열의 리해시를 수행하는 과정이 삽입 과정에 포함된 결과로 보입니다.  
...
두 구현 모두 로드 팩터가 높아지면 하나의 버킷에 포함되는 원소가 늘어 해시 충돌이 일어날 확률이 더 높아집니다.

따라서 로드 팩터 가 높아질 수록 해시 충돌로 인한 탐색 시간의 증가가 결과로 나타나는 겁니다.  
