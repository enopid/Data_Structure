...
100,000개의 고유 정수를 삽입·탐색·삭제한 결과, 탐색·삭제의 경우 커스텀 구현이 약간 빠르고 삽입의 경우 약간 느린 결과가 나왔지만 그 차이는 크지 않았습니다.

기본적으로 모든 연산의 경우 $O(logN)$의 동일한 복잡도를 가지지만, 삽입과 삭제의 경우 회전과 색을 바꾸는 작업과 메모리 할당/해제 작업이 있으므로 실제 비용은 더 클 수 밖에 없습니다.

따라서 삽입과 삭제는 평균적으로 N/2의 크기에서 수행되고 탐색은 N의 크기에서 수행되는 불리한 조건에도 두 구현 모두 탐색의 시간이 비교적 더 빠른 결과를 확인할수있습니다. 
...
다만, 삽입·탐색·삭제 시, 탐색의 경우 삽입·삭제와 비교시 커스텀 구현과 stl 모두 특히나 빠른 경향을 보입니다. 

Hash Table의 경우, 삽입·삭제 시 메모리 관리 비용이 실제 연산을 수행하는 부분보다 훨씬 빨라 삽입의 시간이 상대적으로 빨라져 나타나는 현상으로 보여집니다.

따라서, 메모리 관리 비용을 제거한 탐색 영역의 시간 차이가 실제적인 컨테이너의 연산 비용을 보여준다 할 수 있습니다. 

stl에서 만의 비용 차이가 18배에 달하는 만큼 Red-Black Tree의 정렬 기능이 별도로 필요하지 않은 고유키 관리 상황에서는 해시 테이블을 사용하는 편이 더높은 성능을 보장하는것으로 보입니다.  
