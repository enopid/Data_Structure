...
- 삽입과 삭제 후 색상 변경 및 좌우 회전을 통한 레드블랙 규칙 구현
...
100,000개의 고유 정수를 삽입·탐색·삭제한 결과, 중앙값 기준으로 커스텀 구현은 STL보다 탐색에서 약 8.2%, 삭제에서 약 31.4% 빨랐고 삽입에서는 약 17.4% 느렸습니다.

기본적으로 모든 연산은 $O(\log N)$의 동일한 복잡도를 가지지만, 삽입과 삭제에는 회전과 색상 변경, 메모리 할당·해제 작업이 추가되므로 탐색보다 실제 비용이 커질 수밖에 없습니다.

삽입은 크기가 0에서 N까지 증가하고 삭제는 N에서 0까지 감소하므로 두 연산은 평균적으로 약 N/2 크기의 컨테이너에서 수행됩니다. 탐색은 N개의 원소가 유지된 상태에서 수행되지만, 구조 변경과 메모리 관리가 없어 두 구현 모두 삽입·삭제보다 빠르게 측정됐습니다.
...
아래 값은 각 Ordered 컨테이너의 중앙값을 같은 구현 계열의 Unordered 컨테이너 중앙값으로 나눈 배수입니다.

| 컨테이너 | 삽입 | 탐색 | 삭제 |
| --- | ---: | ---: | ---: |
| 커스텀 구현 | 2.56x | 31.51x | 7.63x |
| STL | 2.76x | 18.28x | 9.14x |

100,000개의 고유 정수를 삽입·탐색·삭제한 결과, Unordered 컨테이너가 모든 측정 단계에서 Ordered 컨테이너보다 빠르게 측정됐습니다.

Hash Table은 평균적으로 삽입·탐색·삭제를 $O(1)$에 처리하므로, 각 연산에 $O(\log N)$이 필요한 Red-Black Tree보다 이번 키 기반 작업에서 유리했습니다.

특히 탐색은 삽입·삭제와 달리 원소의 동적 할당·해제를 수행하지 않으므로 커스텀 구현과 STL 모두 상대적으로 빠르게 측정됐습니다.

Hash Table의 삽입·삭제에는 노드의 동적 할당·해제 비용이 포함되므로, 해시 계산과 버킷 탐색만 수행하는 조회보다 전체 시간이 길게 측정됩니다.

메모리 할당·해제가 포함되지 않는 탐색 결과에서는 해시 테이블과 레드블랙 트리의 조회 경로 차이가 더 직접적으로 나타났습니다.

STL에서도 탐색 중앙값이 약 18.28배 차이 난 만큼, 정렬 순회와 순서 기반 연산이 필요하지 않은 고유 키 관리 상황에서는 해시 테이블이 더 적합한 선택입니다.
