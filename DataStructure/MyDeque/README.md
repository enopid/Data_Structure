...
- 원소 타입: `int`
- 사전 생성한 연산 수: 100,000회
- 측정 횟수: 각 케이스 100회
- 작업: 고정 시드로 생성한 `push_front`, `push_back`, `pop_front`, `pop_back` 혼합 연산
- 삽입/삭제 비율: 약 60/40, 혼합 연산 후 남은 원소는 양 끝에서 교대로 삭제
- 시간 단위: microseconds (`us`)
- 모든 케이스 순서 가중 체크섬: `99,184,490,967,977`

### Release 측정 결과

| 구현 | 청크/블록 크기 | 청크당 int | 평균 | 중앙값 | 최소 | 최대 | std::deque 대비 평균 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `MyDeque<int, 16>` | 16B | 4 | 1,145us | 1,088us | 1,022us | 1,587us | 1.12배 |
| `MyDeque<int, 64>` | 64B | 16 | 869us | 828us | 799us | 1,257us | 0.85배 |
| `MyDeque<int, 256>` | 256B | 64 | 751us | 723us | 682us | 1,449us | 0.73배 |
| `MyDeque<int, 1024>` | 1,024B | 256 | 693us | 677us | 668us | 874us | 0.68배 |
| `MyDeque<int, 4096>` | 4,096B | 1,024 | 707us | 700us | 669us | 847us | 0.69배 |
| `std::deque<int>` | 16B¹ | 4 | 1,025us | 991us | 961us | 1,292us | 기준 |
...
- MSVC `std::deque<int>`도 16Byte 블록을 사용하지만, 블록 포인터 맵은 여유 슬롯을 두고 필요할 때만 확장합니다. 같은 16Byte 설정에서 `MyDeque`의 중앙값은 1,088us, STL은 991us로 `MyDeque`가 약 1.10배 느렸습니다.
- 64Byte 이상의 `MyDeque`는 이 혼합 부하에서 STL보다 낮은 평균과 중앙값을 보였습니다. 1,024Byte 청크의 중앙값이 677us로 가장 낮았습니다.
- 삽입과 삭제를 수행 도중에 섞었기 때문에 청크 경계 순환, 기존 슬롯 재사용, 양방향 접근 비용이 한 번의 측정에 함께 반영됩니다.
- 백그라운드 스케줄링과 메모리 할당 변동의 영향을 줄이기 위해 평균과 함께 중앙값을 확인했습니다.
