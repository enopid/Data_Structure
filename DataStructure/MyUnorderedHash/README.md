...
> `Main.cpp`의 유효성 테스트는 AI의 도움으로 작성하고, Release x64에서 실행해 결과를 확인했습니다. 해시 테이블과 네 어댑터의 자료구조 로직은 직접 구현했습니다.
...
Release x64에서 조건별 15회 반복한 **한 번의 실행 결과**입니다. 시간 단위는 µs이며, 각 회차의 조회 전체 시간에 대한 평균·중앙값입니다.

| 실제 로드 팩터 | 버킷 수 | MyUnorderedSet 평균 / 중앙값 | std::unordered_set 평균 / 중앙값 |
| ---: | ---: | ---: | ---: |
| 0.25 | 32,768 | 25 / 24 µs | 45 / 43 µs |
| 0.5 | 16,384 | 26 / 25 µs | 54 / 51 µs |
| 1.0 | 8,192 | 31 / 31 µs | 46 / 46 µs |
| 2.0 | 4,096 | 42 / 42 µs | 75 / 51 µs |
