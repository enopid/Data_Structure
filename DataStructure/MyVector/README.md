...
```powershell
MyVector.exe --benchmark 10000 5
```

첫 번째 숫자 `10000`은 각 방식에 추가할 원소 수이고, 두 번째 숫자 `5`는 각 방식을 반복 측정할 횟수입니다. 두 번째 숫자를 생략하면 기본 10회 측정합니다. 반복 결과는 평균값, 중앙값, 최솟값과 최댓값으로 출력합니다.

| 방식 | 평균 | 중앙값 | 최소 | 최대 | 재할당 | 최종 용량 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| MyVector Geometric Growth 1.5 (기본값) | 47μs | 46μs | 42μs | 60μs | 23회 | 12,138 |
| MyVector Geometric Growth 2.0 (변경값) | 33μs | 34μs | 32μs | 36μs | 14회 | 16,384 |
| MyVector Linear Growth | 494μs | 495μs | 489μs | 500μs | 1,000회 | 10,001 |
| `std::vector` | 56μs | 58μs | 48μs | 61μs | 24회 | 12,138 |
...
이번 측정에서 MyVector가 `std::vector`보다 빠르게 나타났지만 측정 횟수는 각 방식당 5회입니다. 따라서 해당 수치는 일반적인 우위가 아니라 현재 환경과 입력 조건에서 얻은 결과로 한정합니다.
