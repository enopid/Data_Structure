...
| 구분 | 내용 |
| --- | --- |
| **목표** | STL 컨테이너의 내부 구조를 직접 구현하고, 자료구조별 설계와 성능 특성을 분석합니다. |
| **핵심 구현** | 연속 메모리와 객체 수명 관리, 양방향 iterator, 청크 기반 저장소, Heap, rehash, Red-Black Tree balancing |
| **차별점** | Handle 기반 Priority Queue의 `update`·`erase`·Decrease-Key, 전역 리스트의 연속된 버킷 구간을 사용하는 Hash Table, `malloc`과 placement new를 이용한 객체 수명 직접 관리 |
| **검증** | MSVC STL 결과와의 대조, 무작위 연산 기반 differential test, `Release x64` 반복 benchmark와 평균·중앙값 분석 |

완성된 기능의 수보다 메모리 관리, 데이터 이동, 용량 증가 정책처럼 성능을 결정하는 핵심 원리를 확인하는 데 중점을 두었습니다. 각 자료구조는 가능한 한 독립적으로 구성하여 구현과 분석을 쉽게 읽을 수 있도록 했습니다.
