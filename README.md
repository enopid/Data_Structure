...
완성된 기능의 수를 늘리는 것보다 메모리 관리, 데이터 이동, 용량 증가 정책처럼 자료구조의 성능을 결정하는 핵심 원리를 확인하는 데 목적이 있습니다. 각 자료구조는 가능한 한 독립적으로 구성하여 구현과 분석을 쉽게 읽을 수 있도록 합니다.

## 구현 목록

| 자료구조 | 상태 | 설명 |
| --- | --- | --- |
| [MyVector](DataStructure/MyVector/README.md) | 구현 중 | 연속 메모리 기반 동적 배열 |
| MyList | 준비 중 | 연결 리스트 |
...
- 결과 관리: 원본 측정값과 생성한 그래프는 `Result/`에 분리해 보관합니다.
...
- `MyVector`를 포함한 자료구조의 핵심 로직은 직접 구현했습니다.
- `Main.cpp`의 유효성 테스트 및 벤치마크 코드는 AI를 통해 생성했습니다.
- AI로 생성한 코드는 직접 구현한 자료구조의 동작을 검증하고 결과를 측정하는 보조 도구로만 사용합니다.
